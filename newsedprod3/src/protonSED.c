...
double pionXS(double Ep){
  ...
  double sigmainel=(30.7-0.96*log10(Tp/Tthres)+0.18*pow(log10(Tp/Tthres),2.0))*pow(1.0-pow(Tthres/Tp,1.9),3.0)*1E-27; //inelastic XS in cm^2
...
double getAmax(double Ep){
  ...
  if(Tp<1.0) Amax=5.9*pionXS(Ep)/(Emaxpionlab);
    ...
    Amax=b1*pow(thetap,-b2)*exp(b3*pow(log10(thetap),2.0))*pionXS(Ep)/protonmassGeV;
