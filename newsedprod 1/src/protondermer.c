 ...
 sizegamma=(int)(20+8)*100/STEP;
  ...
  for(i=0;i<=sizegamma;i++){
     ...
     for(j=0;j<protonarraysize;j++){
      ...
      if ((Tp>0.2797E-3)&&(piondata->array[0][i]<Tp)) piondata->array[1][i] +=lightspeedcm*density*(protondistribution[1][j])*DermerXS(Ep,piondata->array[0][i])*dTp/Tp*1E3;//I may need to release the dTpion if the things bug
     }
     }
...
double DermerXS(double Ep, double Epion){
  ...
  else if (pp>=8.0) XSpp=32.0*log(pp)+48.5*pow(pp,-0.5)-59.5;
